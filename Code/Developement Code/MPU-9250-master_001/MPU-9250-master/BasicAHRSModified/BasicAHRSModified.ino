...
//  TWBR = 12;  // 400 kbit/sec I2C speed
  ...
  Serial.println("This sketch is running...");
  digitalWrite(myLed, HIGH);

  uint8_t c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);

  if (c == 0x71) { // WHO_AM_I should always be 0x71a
...
void loop() {
  ...
  MadgwickQuaternionUpdate(q, ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f,  my,  mx, mz);
  //MahonyQuaternionUpdate(q, ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
...
uint8_t readByte(uint8_t address, uint8_t subAddress) {
  ...
  if (Wire.endTransmission(I2C_NOSTOP, 1000) > 0)
    return 0;
  if (Wire.requestFrom(address, (size_t) 1, I2C_NOSTOP, 1000) > 0)
    return Wire.read();                      
  else return 0;         
}
...
void MadgwickQuaternionUpdate(float q[4], float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
...
// measured ones.
void MahonyQuaternionUpdate(float q[4], float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
  float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
  float norm;
  float hx, hy, bx, bz;
