...
#define NICH_CUT_TIME	12 	// Seconds
...
public:
	...
	void buffer(float data, int posCSV) { // Add values to buffer
		char tempBuffer[15];
		sprintf(tempBuffer, "%.2f", data);
		cvsBuffer[posCSV] = String(tempBuffer);
	}
	...
	void buffer(String data, int posCSV) { // Add values to buffer
		cvsBuffer[posCSV] = data;
	}
		...
		switch(state) {
			...
			case S_MONITORING: // Waiting for conditions to arm
				if (aboveArmAlt) {
					if (timerA >= 10*1000) {
						state = S_ARMED;
						cutTimer = 0; // Reset as soon as armed
						timerA = 0;
					}
				}
				else timerA = 0;
			...
			case S_PREPPING: // Waiting for lower cutdown to complete
				if (lowerCut->state == S_COMPLETE || timerA >= NICH_CUT_TIME*1000) executeNow(); // Okay to cutdown, or timeout
	...
	void update() {
		switch(state) {
			...
			case S_PREPPING: // Waiting for upper/lower cutdown to complete
				if (upperCut->state == S_COMPLETE || timer >= 2*NICH_CUT_TIME*1000) executeNow(); // Okay to deploy parachute
...
void setup() {
	...
	parachuteNich.init(GPO_2, 0.7);
	upperCutNich.init(GPO_1, 1.2);
	lowerCutNich.init(GPO_4, 1.8);
...
void loop() {
	...
	if (button.rose == true) parachute.executeNow();
...
void updateClassStatesGPS() {
	...
	//upperCutDown.maxAltIncreased = gps.maxAltIncreased; // Not currently used

	if (gps.altitude >= 1524.0) { // 5k ft
	...
	parachute.belowDeployAlt = (gps.altitude < 3048.0);
	...
	// Lower right corner: 4628.14570, -11702.39442
	// Upper left  corner: 4746.63872, -11951.79980
	if (gps.latitude  >= 4628.14570  && gps.latitude  <= 4746.63872
	 && gps.longitude >= -11951.79980 && gps.longitude <= -11702.39442) {
...
void updateClassStatesBaro() {
	...
	if (barometer.altitude >= 1524.0) { // 5k ft
		lowerCutDown.aboveArmAlt = true;
		upperCutDown.aboveArmAlt = true;
	}
	else {
	...
	parachute.belowDeployAlt = (barometer.altitude < 3048.0);
...
void bufferLogData() {
	if (gps.newData || gps.timeout) {
		barometer.measure(); // Start barometer measurement process
		...
		sdCard.buffer(printState(parachute.state), CSV_PARACHUTE);
		sdCard.buffer(printState(upperCutDown.state), CSV_UPPERCUT);
		sdCard.buffer(printState(lowerCutDown.state), CSV_LOWERCUT);
...
void updateDisplayLights() {
	...
	unsigned int period, onTime;
	char colorCode;

	if (lowerCutDown.state == 3 || upperCutDown.state == 3 || parachute.state == 3) {
		period = 200;
		onTime = 50;
		colorCode = 'y';
	}
	...
	if (lightTimer >= period) {
		lightTimer = 0;
		rgb.color(colorCode);
	}
	else if (lightTimer >= onTime) {
		rgb.color(BLACK);
	}
